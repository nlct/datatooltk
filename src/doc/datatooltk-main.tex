...
\ext{dbtex} format are difficult to edit, but they are by far the
fastest way of loading a \sty{datatool} \gls{database} in \LaTeX.
The \app{datatooltk} application provides a \gls{gui} to make it
easier to edit these files. 

\appname\ can also import data from \ext{csv} files, from Excel
\ext{xls} (but not \ext{xlsx}) or Open Document \ext{ods}
spreadsheets, from \gls{sql} databases (only MySQL is currently
supported) and from \sty{probsoln} databases.  This manual assumes
the user has some knowledge of the \sty{datatool} package. Please
ensure you have at least version 2.15 of \sty{datatool} installed in
your \TeX\ distribution.  (Although the latest version is
recommended.) Note that \sty{datatool} v3.0+ is required to support 
\gls{DBTEX3} or \gls{DTLTEX3} file formats.

The \app{datatooltk} application can be run in either 
batch mode (default, see \sectionref{sec:commandoptions}) 
or \gls{gui} (see \sectionref{sec:gui}).

\begin{information}
The \LaTeX\ document compilation tool \app{arara} (available on
\CTANpkg{arara}) comes with a \app{datatooltk} directive 
...
(The formats are documented in the user manual for \sty{datatool}
v3.0+ but they are essentially \LaTeX\ files designed for use with
\sty{datatool}.) Note the \sty{datatool} v3.0+ is required with 
\gls{DBTEX3} and \gls{DTLTEX3}. Older versions of \sty{datatool}
...
Use case-sensitive comparison when sorting strings.
...
(Default.) Use case-insensitive comparison when sorting strings.
...
\switchdef{missing-console-action}
...
Synonym for \switch{missing-console-action}.
...
\section{Plugins}\label{sec:plugins}
   ...
   \sectionref{sec:preferences}). You will also need to ensure that
...
\chapter{Importing Data}\label{sec:import}
   ...
   \switch{map-tex-specials} (or \switch{literal}) command line option is used or the
   \widget{iosettings.csv.content.literal} option has been selected in the 
   \dialog{preferences} dialog box (see \sectionref{sec:preferences}).
...
In the \widget{preferences.tex} tab you can also specify whether 
non-\TeX\ sources contain literal content or \TeX\ code.
If the \widget{iosettings.csv.content.literal} box is selected,
...
\widget{iosettings.tex.add_map} button, which opens the dialog box
shown in \figureref{fig:addmap}. Note that this table is only
...
\FloatFig[htbp]
 ...
 {Add Mapping Dialog}
    ...
    \widget{iosettings.tex.remove_map}. To edit a mapping,
    select the mapping and click on \widget{iosettings.tex.edit_map}.
...
\sectionentry{preferences.editor}

The \widget{preferences.editor} tab governs the font and style for
the cell editor window (see \sectionref{sec:celleditor}).

The default font used in cell entries is a monospaced font. This can be
changed using the \widget{preferences.editor.font} drop-down menu.
You can also set the font size in the 
\widget{preferences.editor.fontsize} field.
The text \manualtext{colour} and background \manualtext{colour} can be changed 
using the \widget{preferences.editor.cellforeground}
and \widget{preferences.editor.cellbackground} selectors.

Simple syntax highlighting can be enabled or disable by selecting
or deselecting the \widget{preferences.editor.syntax} checkbox.
Enabling this option will show the 
\widget{preferences.editor.highlightcomment} and
\widget{preferences.editor.highlightcs} selectors for comment
...
set with the \widget{preferences.editor.height} and
\widget{preferences.editor.width} widgets. Note that this is a
...
(Use the \widget{preferences.editor} tab for the cell editor display
settings.)
By default, each
...
\widget{preferences.display.celldims} area. These can be changed
as required.
