...
batch mode (default) or \gls{gui} (see \sectionref{gui}).
...
and column identifiers. (See also \sectionref{spaces}.)
...
if \meta{number} is the empty string \code{""}. (See \sectionref{shuffle}.)
...
is assumed.  (See \sectionref{sort}.)
...
\sty{listings} package). For example, in \autoref{ex.verb} 
...
\section{Null Values}\label{null}
...
not necessarily the same as the filename (see \sectionref{fileext}). 
...
but can be changed, as described in \sectionref{editheader}.
...
open, see \sectionref{celleditor}.
...
\dialog{preferences} dialog box (see \autoref{preferences}).
...
dialog box (see \sectionref{celleditor}). You can now scroll through
the cell contents.
...
\section{Cell Editor}\label{celleditor}
   ...
   <b>Important:</b> if you use \sty{datatool}'s \gls{DTLsaverawdb} 
...
\end{verbatim}
   ...
   If you have <tt>arara</tt> version 4.0, there's a rule for \app{datatooltk}:
...
\section{Shuffling the Data}\label{shuffle}
   ...
   <opt tag="compat"/> 1.6 when invoking \app{datatooltk}. For example
...
\verbatiminput{../tests/test-shuffle-level.tex}
   ...
   can invoke \app{datatooltk} in your document <em>before</em> you load the database:
...
\section{Sorting and Shuffling}\label{sortshuffle}
...
<alltt>
<dict tag="appname"/> <opt tag="shuffle"/> <opt tag="in"/> \meta{in-file} <opt tag="output"/> \meta{out-file}
</alltt>
...
<alltt>
<dict tag="appname"/> <opt tag="seed"/> 2013 <opt tag="shuffle"/> <opt tag="in"/> \meta{in-file} <opt tag="output"/> \meta{out-file}
</alltt>
   You will always get the same random ordering <em>provided the original 
   data in \meta{in-file} has remained unchanged</em>. If you want to 
...
\begin{example}{label}{}
   ...
   with a call to \app{datatooltk} <opt tag="shuffle"/> followed by 
...
\verbatiminput{../tests/test-exam.tex}
   ...
   <b>Note:</b> since this overwrites the \sty{datatool} file, you will
...
\begin{example}{label}{}
   ...
   as required, but don't call it <dq>datagidx</dq> as the \sty{datagidx} 
