...
% TODO update this
   ...
   \gls{DTLloaddb} command, \app{datatooltk} can import data with multi-lined
...
\chapter{Application Properties}\label{sec:preferences}
   ...
   \dialog{preferences} dialog box, which has the 
   following tabs.

\widgetdef{preferences.general}[(\figureref{fig:prefgeneral})]
...
\FloatFig[htbp]
 ...
 {General Tab}
     ...
     In the \widget{preferences.general} tab you can specify the start up directory. (The default 
...
\widgetdef{preferences.csv}[(\figureref{fig:prefcsv})]
...
\FloatFig[htbp]
 ...
 {CSV Tab}
     ...
     In the \widget{preferences.csv} tab you can specify the \gls{csv} settings. Some of
...
\widgetdef{preferences.sql}[(\figureref{fig:prefsql})]
...
\FloatFig[htbp]
 ...
 {SQL Tab}
    ...
    In the \widget{preferences.sql} tab, you can specify the \gls{sql} connection information.
...
\widgetdef{preferences.tex}[(\figureref{fig:preftex})]
...
to parse \ext{tex} files. The file encoding for \TeX\ 
files (including \ext{dbtex} files read and saved by
\app{datatooltk}) is set in this tab in the
\widget{preferences.tex.encoding} dropdown list.
...
In the \widget{preferences.tex} tab you can also specify whether or not to map \TeX\ special 
    characters when you import data from \gls{csv} or \gls{sql}.
    If you want the mapping, make sure the \widget{preferences.tex.map}
    box is checked. The mapping table and buttons are hidden if 
    \widget{preferences.tex.map} is unchecked. If it is checked, 
    the performed mappings are listed
    in the table in the tab. To add another mapping, click on the
    \widget{preferences.tex.add_map} button, which opens the 
...
\widgetdef{preferences.currencies}[(\figureref{fig:prefcurrencies})]
...
\FloatFig[htbp]
 ...
 {Currencies Tab}
    ...
    \widget{preferences.currencies} tab. If you add any currencies
    to the list, remember to add them in your document as well with 
    \gls{DTLnewcurrencysymbol}.

\widgetdef{preferences.display}[(\figureref{fig:prefdisplay})]
...
\widgetdef{preferences.language}[(\figureref{fig:preflanguage})]
...
\widgetdef{preferences.plugins}[(\figureref{fig:prefplugins})]
...
\FloatFig[htbp]
 ...
 {Plugins Tab}
    ...
    field of the \widget{preferences.plugins} tab. If it's not on your
...
\label{sec:licence}
   ...
   \item Open CSV \filefmt{opencsv.jar}
   ...
   \item the Java Look and Feel Graphics Repository \filefmt{jlfgr-1\_0.jar}
